...
namespace graph_optimization {
    ...
    void EdgeSE3Prior::compute_residual() {
        auto &&param_i = _vertices[0]->get_parameters();
    ...
    #else
        _residual.block<3,1>(0,0) = 2 * (Qp_.inverse() * Qi).vec();
    #endif
        // translation error
        _residual.block<3,1>(3,0) = Pi - Pp_;
    //    std::cout << residual_.transpose() <<std::endl;
    }

    void EdgeSE3Prior::compute_jacobians() {
